...
namespace Azur.StateMachine
{
    ...
    public interface IStateBuilder<T, TParent> where T : AbstractState, new()
    {
        ...
        IStateBuilder<TState, IStateBuilder<T, TParent>> State<TState>() where TState : AbstractState, new();
        ...
        /// <returns>A new state builder object for the new child state</returns>
        IStateBuilder<TState, IStateBuilder<T, TParent>> State<TState>(string name) where TState : AbstractState, new();
